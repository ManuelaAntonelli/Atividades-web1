...
form.addEventListener('submit', async function(event) {
    ...
    try {
        ...
        if (data.length === 0) {
            ...
            data.sort((a, b) => {
            const nomeA = a.show.name.toLowerCase();
            const nomeB = b.show.name.toLowerCase();
            const termoBusca = query.toLowerCase();

            if (nomeA === termoBusca && nomeB !== termoBusca) return -1;
            if (nomeB === termoBusca && nomeA !== termoBusca) return 1;

            if (nomeA.startsWith(termoBusca) && !nomeB.startsWith(termoBusca)) return -1;
            if (nomeB.startsWith(termoBusca) && !nomeA.startsWith(termoBusca)) return 1;
